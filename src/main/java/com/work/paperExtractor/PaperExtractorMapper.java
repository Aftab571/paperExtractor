...
public class PaperExtractorMapper {
...
//		qArr.add("hash+join+algorithm");
		...
		PaperExtractorMapper demo = new PaperExtractorMapper();
...
//		 Use File.separator as it will work on any OS
		prefs.put("download.default_directory", System.getProperty("user.dir") + File.separator + "ResearchPapers"
				+ File.separator + data.get("venue") + File.separator + data.get("year"));
		prefs.put("plugins.always_open_pdf_externally", true);
		prefs.put("download.prompt_for_download", false);
...
//		PaperExtractorApplication demo = new PaperExtractorApplication();
		...
		// for (int i = 0; i < 1; i++) {
		//
		// driver.get("https://scholar.google.com/scholar?start=" + pageNo + "&q=" +
		// qVal
